...
void CMain(void){
    ...
    paint_string(sheet_control, back_sheet, 0, 0, p,COL8_FFFFFF);

    p = intToHexStr(tss_a.esp);
    paint_string(sheet_control, back_sheet, 0, 16, p, COL8_FFFFFF);

    p = intToHexStr(tss_a.es / 8);
    paint_string(sheet_control, back_sheet, 0, 32,p, COL8_FFFFFF);

    p = intToHexStr(tss_a.cs / 8);
    paint_string(sheet_control, back_sheet, 0, 48,p, COL8_FFFFFF);

    p = intToHexStr(tss_a.ss / 8);
    paint_string(sheet_control, back_sheet, 0, 64,p, COL8_FFFFFF);

    p = intToHexStr(tss_a.ds / 8);
    paint_string(sheet_control, back_sheet, 0, 80,p, COL8_FFFFFF);

    p = intToHexStr(tss_a.gs / 8);
    paint_string(sheet_control, back_sheet, 0, 96, p,COL8_FFFFFF);

    p = intToHexStr(tss_a.fs / 8);
    paint_string(sheet_control, back_sheet, 0, 112, p,COL8_FFFFFF);

    p = intToHexStr(tss_a.cr3);
    paint_string(sheet_control, back_sheet, 0, 128,p, COL8_FFFFFF);
...
void task_b_main(void){
    paint_string(sheet_control,back_sheet , 0, 144, "enter task b",COL8_FFFFFF);
    ...
    for(;;) {
       ...
       io_cli();
        if (fifo8_status(&timerinfo_b) == 0) {
           ...
           io_sti();
           if (i == 123) {
               paint_string(sheet_control, back_sheet, 0, 160, "switch back",COL8_FFFFFF);
               taskswitch8();
           }
