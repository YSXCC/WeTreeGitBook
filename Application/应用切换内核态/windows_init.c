    ...
    for(;;){
        ...
        if(fifo8_status(&task->fifo) == 0){
            ...
            if(data <= 1 && cursor_c >= 0){      //取的是timer的标志码
                ...
                cursor_y = console_new_line(cursor_y,sheet);

                if(strcmp(cmdline,"mem") == 1){
                    char* s = intToHexStr(memorytotal/(1024));
                    paint_string(sheet_control,sheet,16, cursor_y, "free ",COL8_FFFFFF);
                    paint_string(sheet_control,sheet,52, cursor_y, s   , COL8_FFFFFF);
                    paint_string(sheet_control,sheet,126,cursor_y, " KB",COL8_FFFFFF);
                    cursor_y = console_new_line(cursor_y, sheet);
                    ...
                    while(file_info->name[0] != 0){
                        ...
                        paint_string(sheet_control,sheet,16,cursor_y,file_name,COL8_FFFFFF);
                        int offset = 16 + 8 * 15;
                        char* file_size = intToHexStr(file_info->size);
                        paint_string(sheet_control,sheet,offset,cursor_y,file_size,COL8_FFFFFF);
                        cursor_y = console_new_line(cursor_y,sheet);
                        file_info++;
                    }
                    file_info = (FileInfo* )(ADDRESS_DISK_IMG);
                    ...
                    while(file_info->name[0] != 0){
                        ...
                        if(strcmp(name,file_name) == 1){
                            ...
                            for(t = 0; t < size;t++){
                                c[0] = p[t];
                                c[1] = 0;
                                paint_string(sheet_control,sheet,cursor_x,cursor_y,c,COL8_FFFFFF);
                                cursor_x += 8;
                                if(cursor_x == (8 + 240)){
                                    cursor_x = 16;
                                    cursor_y = console_new_line(cursor_y,sheet);
                    ...
                    segment_descriptor(gdt+19,0X0FFFFF,(int)buffer.pBuffer,0x409A);
                    farjmp(0,19*8);

                    //switch to kernel
                    int addr = (int)asm_console_putchar;
                    addr -= get_code32_addr();
                    char* p = intToHexStr(addr);
                    paint_string(sheet_control,back_sheet,0,176,p,COL8_FFFFFF);
                ...
                if(c != 0 && cursor_x < 240){
                    ...
                    // paint_string(sheet_control,sheet,cursor_x,cursor_y,str,COL8_FFFFFF);
                    // cursor_x += 8;

                    g_Console.cursor_x = cursor_x;
                    g_Console.cursor_y = cursor_y;
                    cmdline[cursor_x / 8 - 2] = c;
                    c_console_putchar(c,1);
                    
                    cursor_x = g_Console.cursor_x;
                    cursor_y = g_Console.cursor_y;
