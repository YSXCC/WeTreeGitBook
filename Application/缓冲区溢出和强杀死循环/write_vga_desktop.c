...
extern int key_shift;   //0表示没有shift按下，1表示左shift按下，2表示右shift按下
...
void CMain(void){
    ...
    for(;;){
        ...
        if(fifo8_status(&keyinfo) + fifo8_status(&mouseinfo) 
            ...
            if(data == KEY_CONTROL && key_shift != 0 && task_console_g->tss.ss0 != 0){
                c_console_putstr("kill process");
                io_cli();
                int addr_code32 = get_code32_addr();             
                task_console_g->tss.eip = (int)kill_process - addr_code32;
                io_sti();
            }
