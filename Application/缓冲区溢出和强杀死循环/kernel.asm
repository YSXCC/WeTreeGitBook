...
;自定义中断，向量号0x2DH（是由ICW2决定的）
.02DH:
    Gate        SelectorCode32,  AsmConsolePutCharHandler,0,     DA_386IGate+0x60
...
ExceptionHandler equ _ExceptionHandler - $$
    ...
    ; mov ecx, [0xFE4]    ;获取内存堆栈指针
    ; add ecx, -8
    ; mov [ecx + 4], ss   ;保存中断时的堆栈段
    ; mov [ecx], esp      ;保存中断时的堆栈指针
...
_AsmConsolePutCharHandler:
AsmConsolePutCharHandler equ _AsmConsolePutCharHandler - $$
    push ds
    push es

    pushad  ;记得压两次，不然会因为退不回去，系统崩溃
    ; pushad  ;这个是给kernel_api用的,但是不会从堆栈取出
...
start_app:  ;void start_app(int eip, int cs,int esp, int ds，&(task->tss.esp0))
    ...
    mov eax, [esp + 52]
    mov [eax], esp
    mov [eax + 4], ss   ;把内核的ESP和SS存在TASK的TSS32,是esp0和ss0中，内核堆栈

    ;处理hlt代码位置，数据段和堆栈段，准备切换
    mov eax, [esp + 36] ;eip    ;记住前面还有一个内核的IP
    mov ecx, [esp + 40] ;cs
    mov edx, [esp + 44] ;esp    应用堆栈
    mov ebx, [esp + 48] ;ds

    ;额外数据段和数据段共用
    mov ds, bx
    mov es, bx  ;?不知道为什么不是ss
    ; mov esp, edx
