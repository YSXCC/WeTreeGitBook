...
char transfer_scancode(int data){
    ...
    if(data == 0x2A || data == 0x36 || data == 0xAA || data == 0xB6 || data >= 0x80){   //顺便处理断码
        return 0;
    }
    ...
    if(key_shift == 0 && data < 0x80 && keytable[data] != 0){
        c = keytable[data];
    }else if(key_shift != 0 && data < 0x80 && keytable_shift[data] != 0){
...
void console_task(SHEET* sheet){
    TIMER* time;
    TASK* task = task_now();
    int data, cursor_x = 16,cursor_c = COL8_000000;
        ...
        if(fifo8_status(&task->fifo) == 0){
            ...
            if(data <= 1){      //取的是timer的标志码
                ...
                if(data == 0x0E && cursor_x > 16){//退格键
                    ...
                    if(transfer_scancode(data) != 0 && cursor_x < 240){
                        paint_rectangle(sheet->buf,sheet->sheet_length,COL8_000000,cursor_x,28,cursor_x+7,43);
                        sheet_refresh(sheet_control,sheet,cursor_x,28,cursor_x + 8,44);
                        str[0] = transfer_scancode(data);
                        str[1] = 0;
