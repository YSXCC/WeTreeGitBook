...
void paint_string(unsigned char* vram, int screen_lenth,int start_x,int start_y,char* str,char color){
    for(;*str != 0x00;str++){
        paint_font(vram,screen_lenth,start_x,start_y,systemFont+ *str * 16 ,color);
        start_x += 8;
    }
}
...
void compute_mouse_position(MOUSE_SEND_INFO* mouse_send_info){
    ...
    paint_rectangle(back_buf,screen_info.screen_lenth,COL8_448CBB,0,0,79,15);
    paint_string(back_buf,screen_info.screen_lenth,0,0,"mouse move",COL8_FFFFFF);
}
